...
function runing() {
    ...
    ctx.fillRect(0, 0, canvas.width, canvas.height)            /// 渲染拖尾
...
stop = setInterval(name1, 150)
function reset() {
    document.getElementById('word-10').innerHTML = '';
    i = 0;
    stop = setInterval(name1, 150)
}
function name1(){
    ...
    document.getElementById('word-10').style.color = 'pink';
    document.getElementById('word-10').style.fontSize = '1.2em';
    document.getElementById('word-10').innerHTML = word.slice(0, i++);
    if (i > word.length){
        clearInterval(stop)
